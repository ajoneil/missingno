...
- **When the developer asks about hardware behavior**: Invoke `/research` with a specific question. Present the findings. Propose whether to compare against the code, dig deeper, or move on. For PPU questions, check `receipts/ppu-overhaul/reference/ppu-timing-model-spec.md` first — it is the canonical gate-level hardware reference (collated from dmg-sim). For non-PPU subsystems, gb-ctr (https://gekkio.fi/files/gb-docs/gbctr.pdf) is the primary written reference. For timing questions, also consult `receipts/resources/gb-propagation-delay-analysis/output/` — the race pairs report and operational paths complement the spec's narrative. For behavioral comparison against reference emulators, use gbtrace (prefer SameBoy, then DocBoy, then gambatte — all behavioural references, corroboration not ground truth).
