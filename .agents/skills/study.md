...
- **When the developer asks about hardware behavior**: Invoke `/research` with a specific question. Present the findings. Propose whether to compare against the code, dig deeper, or move on. For timing questions (especially PPU), also consult `receipts/resources/gb-propagation-delay-analysis/output/` directly — the race pairs report and operational paths provide structural analysis of propagation delays that complement behavioral research. For behavioral comparison against reference emulators, use gbtrace to capture and diff execution traces (see AGENTS.md "Execution tracing (gbtrace)").
