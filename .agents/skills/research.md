...
4. **dmg-sim** (`receipts/resources/dmg-sim/`): Gate-level SystemVerilog simulation built from the DMG-CPU B netlist. Running a ROM produces an FST waveform that can be inspected in GTKWave. Use `scripts/dmg-sim-observe.sh <rom> [seconds] [output_dir]` to run a simulation. Because dmg-sim runs are slow and waveform analysis is specialised, **prefer letting the user run dmg-sim and update the DMG Timing Specification** rather than invoking it directly from a research dispatch. Surface spec gaps in your report and stop.
5. **Platform technical docs**: Pan Docs, TCAGBD, hardware manuals. Useful as cross-references and for non-PPU behaviour where the PPU spec doesn't apply.
6. **Hardware test results and analysis**: Documents, blog posts, or wiki pages where someone has measured real hardware behavior with an oscilloscope, logic analyzer, or test ROM and reported the raw results. These are direct observations of the hardware, not interpretations or models.
7. **Propagation delay analysis**: `receipts/resources/gb-propagation-delay-analysis/` provides static analysis of the DMG-CPU die netlist identifying signal races and deep combinatorial paths that cause propagation delay on real hardware. Coverage extends beyond the PPU, though PPU races have the largest observable effects. Especially valuable for one-dot timing discrepancies. Key files in `receipts/resources/gb-propagation-delay-analysis/output/`:
   ...
   - `signal_concordance.md` — Maps netlist 4-letter cell names to Pan Docs register names
...
8. **Cross-emulator execution traces (morepork)**: The `receipts/resources/morepork/` provides tooling for capturing and comparing execution traces across emulators. Tracked emulators: sameboy, docboy, gambatte, missingno — all behavioural, so they corroborate a hardware finding but never ground it. Prefer SameBoy, then DocBoy, then gambatte. Reference traces for 17 test suites (DMG and CGB) are hosted at [ajoneil.github.io/morepork](https://ajoneil.github.io/morepork/) with structured manifests (status under `systems.{dmg,cgb}.{emulator}`).
   - **Capture**: `MOREPORK_PROFILE=gbmicrotest cargo test -p missingno-gb --features morepork -- <test_name>` → writes `receipts/traces/<rom>.morepork`.
   - **Inspect individual traces**: `morepork info <file>` shows summary; `morepork query <file> --where pc=0x0150` finds specific entries; `morepork query <file> --last 20` shows end-of-test state; `morepork frames <file>` shows frame boundaries. For larger tests, individual inspection is often more productive than diffing.
   - **Compare**: `morepork diff <missingno.morepork> <reference.morepork> --sync "lcdc&0x80" --exclude div,tac,if_` reports first divergence and per-field divergence counts. Best for small focused tests (gbmicrotest).
   - **Render**: `morepork render <file> -o <dir>` renders LCD frames from pixel trace data to PNG for visual comparison.
   - **Reference traces**: generate them locally — the published trace blobs predate format v3 and no longer open. In `receipts/resources/morepork/`: `make adapters` once, then `make traces-<suite> EMUS=sameboy` (output under `build/traces/<suite>/`). The Pages manifests (`https://ajoneil.github.io/morepork/tests/{suite}/manifest.json`) remain valid for per-emulator pass/fail status, keyed `systems.{dmg,cgb}.{emulator}`.
   - Profiles are per-suite TOML files in `receipts/resources/morepork/test-suites/*/profile.toml` (e.g. `gbmicrotest`, `blargg`, `mooneye`).
9. **Hardware timing measurements** (`receipts/resources/gb-timing-data/`): Empirical cycle-level timing data from real Game Boy hardware. Contains parameterized measurement campaigns (TOML definitions in `campaigns/`) covering PPU mode timing, sprite penalties, OAM/VRAM lock boundaries, and timer subsystem timing. Results are CSV files with multi-dimensional sweep data. Check the local clone's `campaigns/` directory for available data, and whether a relevant campaign exists for the behavior in question — if it has results, they are definitive hardware measurements. If a relevant campaign exists but has no data yet, note this in your report.
10. **Test suite sources**: Read the assembly/source for relevant test ROMs to understand exactly what they measure. The expected values in a test ROM that passes on hardware are hardware facts — they tell you what the hardware produces, with specific cycle counts and pixel values.
11. **Hardware test harness** (`receipts/resources/slowpeek/`): Programmable harness for running custom SM83 assembly on real Game Boy hardware with cycle-precise interrupt-driven measurements. **Status: functional against missingno emulator; hardware serial bridge in development.** When no existing data source answers the question, note that a Slowpeek test could provide the definitive measurement. Do not attempt hardware mode yet, but do flag when it would be valuable.
...
If the DMG Timing Specification (https://ajoneil.github.io/dmg-timing-spec/) is silent on the question but the behaviour is something a dmg-sim measurement could answer, **stop and report a spec gap**. Do not invoke dmg-sim yourself from a research dispatch (the runs are slow and waveform analysis is specialised — the user owns that). Instead, return a report that:

- Names the specific signal, gate, or timing event that needs measuring (e.g. "the gate-level delay from TEKY↑ to the first TAKA↓ under SCX=3, window enabled")
- References the spec section where the gap lives (e.g. "§6.9 sprite fetch — spec gives a fixed 6-dot penalty but doesn't cover interaction with window re-enable")
...
1. Write your report to the specified output path (Findings / Sources / Confidence / See also). `Confidence` values: `high` (primary source confirmed), `low` (emulator-sourced, unconfirmed), `spec-gap` (PPU question the spec doesn't answer but dmg-sim could — include the specific measurement needed).
