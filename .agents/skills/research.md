...
2. **The DMG Timing Specification** (https://ajoneil.github.io/dmg-timing-spec/): The canonical gate-level hardware reference for the whole DMG SoC (PPU, APU, CPU boundaries, DMA, interrupts, timers), collated from dmg-sim measurements, netlist analysis, and propagation-delay analysis. For any gate-level timing question, **check this first**. Its concordance chapter maps gate names to semantic roles. If the spec doesn't cover the question but a dmg-sim measurement could, stop — see *Reporting spec gaps* below.
...
4. **dmg-sim** (`receipts/resources/dmg-sim/`): Gate-level SystemVerilog simulation built from the DMG-CPU B netlist. Running a ROM produces an FST waveform that can be inspected in GTKWave. Use `scripts/dmg-sim-observe.sh <rom> [seconds] [output_dir]` to run a simulation. Runs are slow and waveform analysis is specialised — prefer surfacing a spec gap (see *Reporting spec gaps* below) over invoking dmg-sim from a research dispatch.
...
7. **Propagation delay analysis**: `receipts/resources/gb-propagation-delay-analysis/` — static netlist analysis of signal races and deep combinatorial paths (PPU races have the largest observable effects; coverage extends beyond it). Especially valuable for one-dot timing discrepancies: match the symptom against `output/race_pairs_report.md` (findings grouped by observable effect); `operational_paths.md` gives per-dot paths by functional area, `signal_concordance.md` maps cell names to Pan Docs names, and the `.json` files are machine-readable for specific signal lookups.
8. **Cross-emulator execution traces (morepork)**: `receipts/resources/morepork/` — capture, query, diff, and render execution traces across sameboy, docboy, gambatte, missingno (all behavioural: they corroborate a hardware finding but never ground it; prefer SameBoy → DocBoy → gambatte). Capture with `MOREPORK_PROFILE=1 cargo test -p missingno-gb --features morepork -- <test_name>` (writes `receipts/traces/<rom>.morepork`); inspect with `morepork info` / `query` / `frames` / `render`. The published CDN corpus is legacy v2 — generate reference traces locally (`make adapters`, then `make traces-<suite> EMUS=sameboy`); the Pages manifests (`https://ajoneil.github.io/morepork/tests/{suite}/manifest.json`) remain valid for per-emulator pass/fail status, keyed `systems.{dmg,cgb}.{emulator}`. Full command detail: `crates/systems/gb/AGENTS.md` (Key Patterns) and the `/compare-traces` skill.
