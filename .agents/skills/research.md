...
8. **Cross-emulator execution traces (gbtrace)**: The `receipts/resources/gbtrace/` provides tooling for capturing and comparing execution traces across emulators. Tracked emulators: sameboy, docboy, gambatte, missingno — all behavioural, so they corroborate a hardware finding but never ground it. Prefer SameBoy, then DocBoy, then gambatte. Reference traces for 17 test suites (DMG and CGB) are hosted at [ajoneil.github.io/gbtrace](https://ajoneil.github.io/gbtrace/) with structured manifests (status under `systems.{dmg,cgb}.{emulator}`).
   ...
   - **Reference traces**: Manifests are on GitHub Pages; trace blobs are on a DigitalOcean Spaces CDN (two different hosts). Fetch a manifest with `curl -s https://ajoneil.github.io/gbtrace/tests/{suite}/manifest.json`, then download a trace blob (note the `_{system}_` segment — `cgb` or `dmg`) from `https://gbtrace.syd1.cdn.digitaloceanspaces.com/tests/{suite}/{test}_{emulator}_{system}_{status}.gbtrace`.
   - Profiles are per-suite TOML files in `receipts/resources/gbtrace/test-suites/*/profile.toml` (e.g. `gbmicrotest`, `blargg`, `mooneye`).
9. **Hardware timing measurements** (`receipts/resources/gb-timing-data/`): Empirical cycle-level timing data from real Game Boy hardware. Contains parameterized measurement campaigns (TOML definitions in `campaigns/`) covering PPU mode timing, sprite penalties, OAM/VRAM lock boundaries, and timer subsystem timing. Results are CSV files with multi-dimensional sweep data. **Status: data collection in progress — the local clone's `campaigns/` directory is currently empty.** Check whether a relevant campaign exists for the behavior in question — if it has results, they are definitive hardware measurements. If a relevant campaign exists but has no data yet, note this in your report.
