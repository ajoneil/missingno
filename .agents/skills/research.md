...
2. **PPU timing model spec** (`receipts/ppu-overhaul/reference/ppu-timing-model-spec.md`): Canonical hardware reference for the DMG PPU, collated from dmg-sim measurements, netlist analysis, and propagation-delay analysis. For any PPU timing question, **check this first**. Companion files in the same directory: `ppu-signal-concordance.md` (gate name ↔ semantic role) and `spec-conventions.md` (authoring conventions). **If the spec doesn't cover the question but a dmg-sim measurement could, stop and report this to the caller as a "spec gap":** list the specific signal or timing event that needs measuring so the spec can be extended. Do not silently fall back to emulator source to fill the gap.
...
If the PPU timing model spec (`receipts/ppu-overhaul/reference/ppu-timing-model-spec.md`) is silent on the question but the behaviour is something a dmg-sim measurement could answer, **stop and report a spec gap**. Do not invoke dmg-sim yourself from a research dispatch (the runs are slow and waveform analysis is specialised — the user owns that). Instead, return a report that:
