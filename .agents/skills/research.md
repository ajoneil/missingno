...
8. **Cross-emulator execution traces (gbtrace)**: The `receipts/resources/gbtrace/` provides tooling for capturing and comparing execution traces across emulators. Tracked emulators: gambatte, gateboy, docboy, missingno, sameboy. **Prefer the GateBoy adapter** — die-photo-derived, closest to hardware. Fall back to Gambatte / SameBoy / DocBoy only when GateBoy doesn't pass the test or has no trace for it. Reference traces for 15 test suites are hosted at [ajoneil.github.io/gbtrace](https://ajoneil.github.io/gbtrace/) with structured manifests.
