...
2. **Consider existing data sources first.** Before adding code instrumentation, check whether the question can be answered without modifying code. The sources below are the **Game Boy (DMG/CGB)** set; for **VCS** the gate-level analog is **Sim2600** (`receipts/resources/Sim2600/`, run headless via `scripts/sim2600-observe.sh` → per-half-clock VCD) for any CPU or TIA hypothesis (transistor-level, slow — like dmg-sim, prefer surfacing the specific wire/half-clock to the user rather than running a long capture from an instrument dispatch), with the schematics + 6532 datasheet for RIOT and the VCS morepork adapters (Stella/Gopher2600/MAME) for state-at-boundary checks. See `crates/missingno-<core>/AGENTS.md`.
   - **The DMG Timing Specification** (https://ajoneil.github.io/dmg-timing-spec/): For PPU hypotheses about gate-level timing, the spec may already document the expected behaviour. If so, you don't need to measure missingno to confirm the hardware expectation — you only need to measure missingno to find where it diverges. If the spec is silent on a relevant detail, flag the spec gap in your report so the user can run dmg-sim and extend the spec.
   - **morepork**: If the hypothesis can be tested by examining CPU/PPU/timer/interrupt state at instruction boundaries, a morepork capture may answer the question. Capture with `MOREPORK_PROFILE=gbmicrotest` (or the appropriate suite profile), then query with `morepork query <file> --where pc=0x0150 --context 5`. Use `morepork query <file> --last 20` for end-of-test state. Use `morepork render <file> -o <dir>` for visual inspection of PPU output. Prefer SameBoy reference traces when diffing against a reference, then DocBoy, then gambatte — all are behavioural references, not ground truth.
   - **Hardware timing data** (`receipts/resources/gb-timing-data/campaigns/`): Check if a measurement campaign covers the behavior being tested. Campaign CSV data provides cycle-level hardware measurements that may directly answer the hypothesis. Check the clone's `campaigns/` directory for available data.
   - **dmg-sim**: For gate-level signal observations not covered by the DMG Timing Specification, dmg-sim produces an FST waveform viewable in GTKWave. Runs are slow and waveform analysis is specialised — **prefer surfacing the need to the user** with the specific signal to observe, rather than invoking dmg-sim from an instrument dispatch.
   - **Slowpeek** (`receipts/resources/slowpeek/`): If the hypothesis requires measuring a specific hardware behavior not covered by existing data, note that a Slowpeek sweep test would provide the definitive answer. Check whether the hardware serial path is available before attempting a hardware run; flag when a sweep would be useful.
   - Use `eprintln!` instrumentation only for internal missingno state not visible through the data sources above (e.g. FIFO contents, fetcher phases, mid-instruction timing, DFF pending values).
