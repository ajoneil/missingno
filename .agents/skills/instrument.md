...
2. **Consider existing data sources first.** Before adding code instrumentation, check whether the question can be answered without modifying code. The sources below are the **Game Boy (DMG/CGB)** set; for **VCS** the gate-level analog is **Sim2600** (`receipts/resources/Sim2600/`) for any CPU or TIA hypothesis (transistor-level, slow — like dmg-sim, prefer surfacing the specific wire/half-clock to the user rather than running it from an instrument dispatch), with the schematics + 6532 datasheet for RIOT and the VCS gbtrace adapters (Stella/Gopher2600/MAME) for state-at-boundary checks. See `crates/missingno-<core>/AGENTS.md`.
