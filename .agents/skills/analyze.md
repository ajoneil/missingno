...
1b. **For PPU behaviour analysis, consult the DMG Timing Specification** (https://ajoneil.github.io/dmg-timing-spec/). It is the canonical hardware reference (collated from dmg-sim measurements) for the DMG PPU. Match the observed behaviour against the spec's description of the relevant gate / signal / mode transition. If the observed behaviour contradicts the spec, the emulator is wrong (default assumption); if the spec is silent on a detail a dmg-sim run could cover, flag it as a spec gap in your conclusion so the user can extend the spec.
