...
- **Full suite**: Run `./scripts/test-report-gb.sh --diff` to check for regressions (no saved baseline? run `--save-baseline` first, then apply your changes, then `--diff`; reports land in `receipts/test-reports/gb/`). Use the `-gbc` / `-vcs` variant matching the core; changes outside the cores use the gates in AGENTS.md's Testing section (`missingno-session --all-features`, `missingno-debugger --features mcp`, `missingno` for the app). **The steady-state baseline is all-green**, so **any** test that fails after your change is a regression — there is no standing failure set to subtract. Wrap suite runs in a timeout after disruptive changes (`timeout 1500 …` per the Testing section) and treat a large overrun as a hang: kill it and bisect with single-test probes.
