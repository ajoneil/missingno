...
- **Full suite**: Run `./scripts/test-report-gb.sh --diff` to check for regressions (requires a saved baseline — if none exists, run `./scripts/test-report-gb.sh --save-baseline` first, then apply your changes, then `--diff`). The report is saved automatically to `receipts/test-reports/gb/`. Use `test-report-gbc.sh` instead for changes scoped to `missingno-gbc`, and `test-report-vcs.sh` for `missingno-vcs`. Changes outside the cores have their own gates: `cargo test -p missingno-session --all-features` (its default features leave the tool surface off, so a plain run silently skips every tool and attach test), `cargo test -p missingno-debugger --features mcp`, and `cargo test -p missingno` for the app. **The steady-state baseline is all-green** and the gate is simple: after your change the suite must still be fully green. **Any** test that fails after your change is a regression by definition — there is no standing failure set to subtract. **Wrap suite runs in a timeout** after disruptive changes (`timeout 1500 ./scripts/test-report-gb.sh --diff`, or the `-gbc` variant); interrupt/halt-adjacent changes can hang the emulator, so treat a large overrun as a hang, kill it, and bisect with single-test probes.
