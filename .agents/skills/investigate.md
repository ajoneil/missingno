...
- After capturing the baseline, **check available data sources before generating new data.** In order:
  1. **Hardware timing data** (`receipts/resources/gb-timing-data/campaigns/`): Check if a measurement campaign covers the behavior. Campaign CSV data provides definitive cycle-level measurements from real hardware. If a relevant campaign exists but has no results yet, note that it would be valuable when complete.
