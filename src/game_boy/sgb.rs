...
impl Sgb {
    ...
    pub(crate) fn save_state(&self) -> super::save_state::SgbState {
        use super::save_state::{SgbPaletteState, SgbState};
        SgbState {
            palettes: self
                .palettes
                .iter()
                .map(|p| SgbPaletteState {
                    colors: p.colors.map(|c| c.0),
                })
                .collect(),
            attribute_map: self
                .attribute_map
            ...
            system_palettes: self
                ...
                .collect(),
            attribute_files: self
                .attribute_files
                .iter()
                .map(|atf| atf.cells.iter().map(|row| row.to_vec()).collect())
                .collect(),
            mask_mode: match self.mask_mode {
                MaskMode::Disabled => 0,
                MaskMode::Freeze => 1,
                MaskMode::Black => 2,
                MaskMode::BackdropColor => 3,
            },
            player_count: self.player_count,
            current_player: self.current_player,
            prev_p14_p15_both_low: self.prev_p14_p15_both_low,
        }
    }

    pub(crate) fn from_state(state: super::save_state::SgbState) -> Self {
        let palettes = {
            let mut pals = [SgbPalette::default(); 4];
            for (i, sp) in state.palettes.iter().enumerate().take(4) {
                pals[i] = SgbPalette {
                    colors: sp.colors.map(Rgb555),
                };
            }
            pals
        };
        ...
        for (y, row) in state.attribute_map.iter().enumerate().take(18) {
            for (x, &val) in row.iter().enumerate().take(20) {
                attribute_map.cells[y][x] = val;
            }
        }

        let system_palettes = state
            .system_palettes
            .iter()
            .map(|sp| SgbPalette {
                colors: sp.colors.map(Rgb555),
        ...
        let attribute_files = state
            .attribute_files
            .iter()
            .map(|atf_data| {
                let mut atf = AttributeMap::new();
                for (y, row) in atf_data.iter().enumerate().take(18) {
                    for (x, &val) in row.iter().enumerate().take(20) {
                        atf.cells[y][x] = val;
                    }
                }
        ...
        let mask_mode = match state.mask_mode {
            0 => MaskMode::Disabled,
            1 => MaskMode::Freeze,
            2 => MaskMode::Black,
            _ => MaskMode::BackdropColor,
        };
        ...
        Self {
            ...
            command_state: CommandState::Idle,
            prev_p14_p15_both_low: state.prev_p14_p15_both_low,
            last_screen: Screen::new(),
            pending_transfer: None,
        }
    }
