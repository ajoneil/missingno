...
impl Mapper<'_> {
    pub fn new<'a>(mmu: &'a mut Mmu, video: &'a mut Video, timers: &'a mut Timers) -> Mapper<'a> {
        Mapper { mmu, video, timers }
    }

    pub fn read(&self, address: u16) -> u8 {
        self.mmu.read(address, self.video, self.timers)
    }
    ...
    pub fn read_word(&self, address: u16) -> u16 {
        self.mmu.read_word(address, self.video, self.timers)
    }
    ...
    pub fn write(&mut self, address: u16, val: u8) {
        self.mmu.write(address, val, self.video, self.timers)
    }

    pub fn write_word(&mut self, address: u16, val: u16) {
        self.mmu.write_word(address, val, self.video, self.timers)
    }
}
...
impl Mmu {
    ...
    pub fn read(&self, address: u16, video: &Video, timers: &Timers) -> u8 {
    ...
    pub fn read_word(&self, address: u16, video: &Video, timers: &Timers) -> u16 {
        self.read(address, video, timers) as u16
            + (self.read(address + 1, video, timers) as u16 * 0x100)
    }

    pub fn write(&mut self, address: u16, val: u8, video: &mut Video, timers: &mut Timers) {
        if video.dma_transfer_in_progess() && !(0xff80..0xfffe).contains(&address) {
            return;
        }
        ...
        match address {
            ...
            0xff40..=0xff4b => video.write(address, val, &self, timers),
    ...
    pub fn write_word(&mut self, address: u16, val: u16, video: &mut Video, timers: &mut Timers) {
        self.write(address, (val & 0xff) as u8, video, timers);
        self.write(address + 1, (val >> 8) as u8, video, timers);
    }
