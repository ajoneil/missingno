...
use audio::{Channel, ChannelState};
use hsync::{Beam, HSyncCounter};
use missingno_core::waveform::WaveRing;
use objects::{
    Ball, BallState, Missile, MissileState, Player, PlayerState, Playfield, PlayfieldState,
};
...
impl Tia {
    ...
    pub(crate) fn capture(&self) -> TiaState {
        ...
        let movables = |m: &Movables<bool>| [m.p0, m.p1, m.m0, m.m1, m.bl];
        let hm = |m: &Movables<u8>| [m.p0, m.p1, m.m0, m.m1, m.bl];
        TiaState {
            vsync: self.vsync,
            vblank: self.vblank,
            cpu_ready: self.cpu_ready,
            wsync_reset_hold: self.wsync_reset_hold,
            hsync_position,
            hblank_release,
            color_p0: self.color_p0,
            color_p1: self.color_p1,
            color_pf: self.color_pf,
            color_bk: self.color_bk,
            ...
            mot_more_movement: movables(&self.motion.more_movement),
            mot_hm_values: hm(&self.motion.hm_values),
            mot_captured_hm: hm(&self.motion.captured_hm_values),
            hblank_ext_pending_active: self.hblank_extension_pending.is_some(),
            hblank_ext_pending: self.hblank_extension_pending.unwrap_or(0),
            hblank_ext_armed: self.hblank_extension_armed,
            seam_lookahead: movables(&self.seam_lookahead),
            collisions: self.collisions,
            player0: self.player0.capture(),
            player1: self.player1.capture(),
            missile0: self.missile0.capture(),
            missile1: self.missile1.capture(),
            ball: self.ball.capture(),
            playfield: self.playfield.capture(),
            audio: [self.audio[0].capture(), self.audio[1].capture()],
            triggers: self.triggers,
            trigger_latch_enabled: self.trigger_latch_enabled,
            trigger_latches: self.trigger_latches,
            pot_position: std::array::from_fn(|i| {
                (self.pots[i].position.clamp(0.0, 1.0) * u16::MAX as f32).round() as u16
            }),
            pot_countdown: std::array::from_fn(|i| self.pots[i].countdown),
            pot_dumped: self.pot_dumped,
        }
    }
    ...
    pub(crate) fn restore(&mut self, s: &TiaState) {
        ...
        self.cpu_ready = s.cpu_ready;
        self.wsync_reset_hold = s.wsync_reset_hold;
        self.color_p0 = s.color_p0;
        self.color_p1 = s.color_p1;
        self.color_pf = s.color_pf;
        self.color_bk = s.color_bk;
        self.playfield_priority = s.playfield_priority;
        ...
        let set_bools = |m: &mut Movables<bool>, a: [bool; 5]| {
            m.p0 = a[0];
            m.p1 = a[1];
            m.m0 = a[2];
            m.m1 = a[3];
        ...
        let set_u8s = |m: &mut Movables<u8>, a: [u8; 5]| {
            m.p0 = a[0];
            m.p1 = a[1];
            m.m0 = a[2];
            m.m1 = a[3];
        ...
        set_bools(&mut self.motion.more_movement, s.mot_more_movement);
        set_u8s(&mut self.motion.hm_values, s.mot_hm_values);
        set_u8s(&mut self.motion.captured_hm_values, s.mot_captured_hm);
        self.hblank_extension_pending = s.hblank_ext_pending_active.then_some(s.hblank_ext_pending);
        self.hblank_extension_armed = s.hblank_ext_armed;
        set_bools(&mut self.seam_lookahead, s.seam_lookahead);
        self.collisions = s.collisions;
        self.player0.restore(&s.player0);
        self.player1.restore(&s.player1);
        self.missile0.restore(&s.missile0);
        self.missile1.restore(&s.missile1);
        self.ball.restore(&s.ball);
        self.playfield.restore(&s.playfield);
        self.audio[0].restore(&s.audio[0]);
        self.audio[1].restore(&s.audio[1]);
        self.triggers = s.triggers;
        self.trigger_latch_enabled = s.trigger_latch_enabled;
        self.trigger_latches = s.trigger_latches;
        for i in 0..4 {
            self.pots[i].position = s.pot_position[i] as f32 / u16::MAX as f32;
            self.pots[i].countdown = s.pot_countdown[i];
        }
        self.pot_dumped = s.pot_dumped;
