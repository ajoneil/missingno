...
of it (see `crates/systems/gbc/AGENTS.md` for the CGB delta). The shared
...
3. **dmg-sim** (https://github.com/msinger/dmg-sim): Gate-level SystemVerilog simulation built from the DMG-CPU B netlist (Icarus Verilog). Run with `scripts/dmg-sim-observe.sh <rom> [seconds] [output_dir]` to capture an FST waveform (viewable in GTKWave; the script prints where it wrote). Use when the DMG Timing Specification lacks detail it should contain — and update the spec afterwards. Runs are slow and waveform analysis is specialised — **prefer surfacing the specific signal to observe to the user** rather than invoking dmg-sim directly.
4. **Propagation delay analysis**: Signal races and deep combinatorial paths. See Key Patterns below.
5. **Hardware documentation**: Pan Docs, TCAGBD, hardware manuals. Useful for non-PPU behaviour and as cross-reference.
6. **Cross-emulator execution traces** (morepork): behavioural emulators (SameBoy, DocBoy, gambatte, missingno) traced per test suite. Use for both `diff` and individual inspection. References corroborate but never ground a hardware finding — prefer SameBoy, then DocBoy, then gambatte. Reference traces are generated locally (`make traces-<suite> EMUS=...` in the morepork clone). See Key Patterns below.
7. **Hardware timing measurements** (https://github.com/ajoneil/gb-timing-data): Empirical cycle-level data from real hardware via Slowpeek. Campaigns cover PPU timing (mode 3 duration, sprite penalties, OAM/VRAM lock boundaries) and timer subsystem timing (DIV phase, TIMA increment). Results are CSV files with multi-dimensional sweep data. Check the repo's `campaigns/` directory for available data before planning around this source.
8. **Test ROM sources**: Assembly source reveals exactly what tests measure and what expected values mean.
9. **Hardware test harness** (https://github.com/ajoneil/slowpeek): Programmable harness for cycle-precise measurements on real Game Boy hardware via interrupt-driven sweeps. Note when a Slowpeek test would provide the definitive answer; check whether the hardware serial bridge is available before attempting hardware mode.
...
`Console::step()` (`crates/systems/gb/src/execute.rs`) runs `step_instruction()`, which loops `execute_tcycle()` — one CPU T-cycle, its rise then its fall, per call — until the CPU returns to an instruction boundary. The CPU's M-cycle machinery lives in `crates/systems/gb/src/cpu/mcycle/`, split by role: `fetch.rs` (opcode/operand fetch and `Instruction::decode`, plus the free fn `operand_count()`), `scheduler.rs` (M-cycle/T-cycle sequencing, bus arbitration parking), `execute.rs` (per-phase execution steps), `isr.rs` (interrupt dispatch), `build.rs` (action sequences per instruction type), `apply.rs` (pure CPU mutations — ALU, flags, DAA), `types.rs` (`BusAction` and friends). Read the module before trusting any finer-grained claim here.
...
Every source's rank, role, and location is stated once, in the ground-truth
hierarchy above; the two sibling projects with tooling of their own —
`gb-propagation-delay-analysis` and `morepork` — are detailed under Key
Patterns.
