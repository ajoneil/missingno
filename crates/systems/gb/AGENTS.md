...
- **Enum-based MBC dispatch**: `Mbc` enum in `crates/systems/gb/src/cartridge/mbc/mod.rs`, one variant per supported mapper (`NoMbc`, `Mbc1`, `Mbc2`, `Mbc3`, `Mbc5`, `Mbc6`, `Mbc7`, `Huc1`, `Huc3`, `DbzTrans`), selected in `Cartridge::new` from cartridge header byte 0x147 — except `DbzTrans`, an unlicensed cart that declares MBC5 and is recognised by title instead. ROM data is owned by `Cartridge` and passed to MBC `read()` methods as `&[u8]`.
