...
impl Machine for Sg1000System {
    ...
    fn capture_trace(sg: &mut Sg1000, path: &std::path::Path) -> Option<Frame> {
        #[cfg(feature = "morepork")]
        {
            ...
            let budget = 2 * tstates_per_frame(sg.standard());
            let mut elapsed = 0;
            while elapsed < budget {
                tracer.capture(sg).ok()?;
                sg.step_instruction();
                elapsed += sg.cpu.bus_trace().len() as u32;
