...
fn set_vdp_register(asm: &mut Z80Asm, register: u8, value: u8) {
    asm.ld_a(value);
    asm.out_port(0xBF);
...
fn backdrop_frame_carries_cram_snapshot() {
    ...
    let mut sms = Sms::new(&asm.into_rom(0x4000)).unwrap();
...
fn sega_mapper_banks_slot_one() {
    ...
    let mut rom = asm.into_rom(0x4000);
...
fn v_counter_wraps_the_ntsc_gap() {
    let mut sms = Sms::new(&Z80Asm::new().into_rom(0x4000)).unwrap();
...
fn set_vram_address(asm: &mut Z80Asm, address: u16) {
...
fn write_vram(asm: &mut Z80Asm, address: u16, bytes: &[u8]) {
...
fn write_cram(asm: &mut Z80Asm, index: u8, value: u8) {
    asm.ld_a(index);
    asm.out_port(0xBF);
    asm.ld_a(0xC0);
    asm.out_port(0xBF);
...
fn background_tile_renders_at_the_origin() {
    ...
    let frame = frame_after_setup(asm.into_rom(0x4000));
