//! The board's clock grid: dividing one crystal into the rates its chips
//! run at.
//!
//! The board owns time. It ticks at its crystal and every chip is a client
//! at its own divisor — the 6502's bus cycle, the Z80's T-state, the VDP's
//! dots — so a bus access issued inside a tick lands against a world that
//! has advanced to that instant. Each chip's stepping and bus surface stay
//! in its own crate; what is shared here is only the division.
