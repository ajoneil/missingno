...
macro_rules! vdp_test {
    ...
    ($body:ident, $name:ident, $path:literal, ntsc_only) => {
        #[test]
        fn $name() {
            crate::testbench::assert_ntsc_only($path, standard!($body));
        }
    };
}
...
macro_rules! corpus {
    ($body:ident) => {
        ...
        mod modes {
            ...
            vdp_screenshot!($body, undoc_text_multicolor, "modes/undoc-text-multicolor.sg");
            vdp_screenshot!(
                $body,
        ...
        mod timing {
            ...
            vdp_test!($body, cadence_8match, "timing/cadence-8match.sg");
            vdp_test!($body, f_edge_locator, "timing/f-edge-locator.sg");
            vdp_test!($body, f_race, "timing/f-race.sg");
            vdp_test!($body, gi_burst, "timing/gi-burst.sg");
            vdp_test!($body, gii_sweep, "timing/gii-sweep.sg");
            vdp_test!($body, line0_sweep, "timing/line0-sweep.sg");
            vdp_test!($body, line187_sweep, "timing/line187-sweep.sg");
            vdp_test!($body, line96_sweep, "timing/line96-sweep.sg");
            vdp_test!($body, m1_split_sweep, "timing/m1-split-sweep.sg");
            vdp_test!($body, match_sweep, "timing/match-sweep.sg");
            vdp_test!($body, mc_sweep, "timing/mc-sweep.sg");
            vdp_test!($body, onset_blank, "timing/onset-blank.sg");
            vdp_test!($body, onset_burst, "timing/onset-burst.sg");
            vdp_test!($body, phantom_burst, "timing/phantom-burst.sg");
            vdp_test!($body, phase_sweep, "timing/phase-sweep.sg");
            vdp_test!($body, satkill_sweep, "timing/satkill-sweep.sg");
            // Sidecar budget 1400: ~550 frames of sweep + the per-cell map compare.
            vdp_test!($body, scan_cadence, "timing/scan-cadence.sg", frames = 1600);
            vdp_test!($body, steal_and, "timing/steal-and.sg");
            vdp_test!($body, steal_prime, "timing/steal-prime.sg");
            vdp_test!($body, steal_raw, "timing/steal-raw.sg");
            vdp_test!($body, steal_sweep, "timing/steal-sweep.sg");
            vdp_test!($body, steal15_raw, "timing/steal15-raw.sg");
            ...
            vdp_test!($body, undoc_tmc_sweep, "timing/undoc-tmc-sweep.sg");
        }
    };
}
