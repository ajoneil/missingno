//! The VDP's memory decoded into the system-agnostic graphics vocabulary
//! ([`missingno_core::graphics`]).
//!
...
//! indices under the datasheet palette. Sprites are the other way round — the
//! attribute owns the colour, not the pattern — so their patterns stay one bit
//! deep and each object carries its colour nibble as its palette selector.
//!
...
use crate::inspect::ti_palette;
use crate::{ACTIVE_LINES, ACTIVE_WIDTH, Mode, SPRITE_TERMINATOR, Vdp};
...
/// The graphics surfaces the VDP's memory decodes to, read live.
pub fn graphics_view(vdp: &Vdp) -> GraphicsView {
