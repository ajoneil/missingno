//! The TMS9918A at register level, as a debugger shows it: where the raster
//! stands, what the status latches hold, the five tables R2-R6 point at, and
//! the backdrop R7 paints — plus the datasheet palette colour indices resolve
//! through.
...
use crate::{ACTIVE_LINES, Mode, PALETTE, Standard, Vdp};
...
/// The chip as its section reads it: the raster position, the status byte, the
/// register file and what the registers select.
#[derive(Clone)]
pub struct VdpView {
    ...
    pub status: u8,
    pub registers: [u8; 8],
    pub layout: VdpLayout,
}

impl VdpView {
    pub fn of(vdp: &Vdp) -> VdpView {
        VdpView {
            standard: vdp.standard(),
            line: vdp.line(),
            dot: vdp.dot(),
            status: vdp.peek_status(),
            registers: *vdp.registers(),
            layout: VdpLayout::of(vdp),
        }
    }
}

/// The palette every frame and every graphics surface hands out, built once.
pub fn ti_palette() -> Arc<[RGB8]> {
...
pub fn ti_colour(index: u8) -> RGB8 {
    RGB8::from(PALETTE[index as usize & 0x0F])
}

pub fn section(state: &VdpView) -> Section {
    ...
    Section {
        ...
        blocks: vec![
            ...
            SectionBlock::Rule,
            SectionBlock::Table(status_table(state.status)),
            SectionBlock::Rows(vec![
                Row::value("scan", format!("{:02}", state.status & SCAN_COUNTER_MASK))
...
#[cfg(test)]
mod tests {
    use missingno_core::inspect::Row;
    ...
    fn vdp_section_reports_all_eight_registers() {
        let view = VdpView {
            registers: [0x00, 0x60, 0x0E, 0xFF, 0x03, 0x76, 0x03, 0x01],
            ..VdpView::of(&Vdp::new(Standard::Ntsc))
        };
        let section = section(&view);
    ...
    fn vdp_section_shows_the_tables_the_registers_select() {
        ...
        let section = section(&VdpView::of(&vdp));
        let rows = rows(&section);
